...
.setting-title {
  color: var(--rooms-header-text-color);
  font-size: 14px;
  margin-bottom: 6px;
}
...
.settings-window {
  ...
  background: var(--timeline-bg-color); border-radius: 8px;
...
.settings-sidebar {
  width: 220px;
  background: var(--spaces-bg-color);
  padding: 8px 10px;
  flex-shrink: 0;
}
...
.settings-tab:hover { background: rgba(255, 255, 255, 0.06); color: #dbdee1; opacity: 1; }
.settings-tab.is-active { background: rgba(255, 255, 255, 0.1); color: #fff; opacity: 1; }
...
.settings-heading {
  color: #f2f3f5; margin-top: 0; margin-bottom: 0; font-size: 20px;
}
...
.profile-name { color: #f2f3f5; font-size: 24px; font-weight: bold; }
.profile-id { color: #b5bac1; font-size: 16px; margin-top: 4px; opacity: 0.70; }
...
.settings-section .verification-description {
  display: none;
}

.settings-section.plugins-view-wrapper {
  max-height: 100vh;
  overflow-y: auto;
}
