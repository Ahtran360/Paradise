...
(def submit-extension
  (.create Extension
           #js {:name "submitExtension"
                :addOptions (fn [] #js {:onSend nil :onEditLast nil})
...
(defn tiptap-component [^js props]
  (let [active-id    (.. props -children -activeId)
        ...
        latest-cbs   (react/useRef #js {:onSend on-send :onFiles on-files :onEditLast on-edit-last})
        _            (set! (.-current latest-cbs) #js {:onSend on-send :onFiles on-files :onEditLast on-edit-last})
        editor (useEditor
