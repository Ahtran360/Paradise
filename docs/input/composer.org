...
(def submit-extension
  (.create Extension
           #js {:name "submitExtension"
                :priority 1000
                :addOptions (fn [] #js {:onSend nil :getEnterForNewLine nil})
...
(defn tiptap-component [^js props]
  (let [active-id    (.. props -children -activeId)
        ...
        latest-cbs        (react/useRef #js {:onSend on-send :onFiles on-files :enterForNewLine enter-for-newline})
        _                 (set! (.-current latest-cbs) #js {:onSend on-send :onFiles on-files :enterForNewLine enter-for-newline})
        editor (useEditor
