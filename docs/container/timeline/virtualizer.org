...
.timeline-content-wrapper {
    ...
    min-width: 0;
    min-height: 0;
    margin-left: var(--chat-avatar-col-w);
}
...
.timeline-message.is-mentioned {
    background-color: color-mix(in srgb, #f5a623 10%, transparent);
    border-left: 2px solid #f5a623;
    padding-left: calc(var(--chat-padding-x) / 2 - 2px);
}

.timeline-message.is-mentioned:hover {
    background-color: color-mix(in srgb, #f5a623 15%, transparent);
}

.swipe-foreground.timeline-message.is-message {
    padding-left: 12px;
    padding-right: 8px;
}
...
.timeline-reply-banner {
    ...
    padding: 4px 10px;
    border-radius: 6px;
    border-left: 2px solid var(--accent-color, #aaa);
    cursor: pointer;
