...
(def ^:private media-max-w 600)
(def ^:private media-max-h 400)
(def ^:private media-min-w 10)
(def ^:private media-min-h 10)

(defn fit-media-dims
  "Scale natural dims (w x h) into the media bounds preserving aspect ratio.
   Caps at 600x400, floors at 10x10, never enlarges past natural size."
  [w h]
  (let [w     (max 1 (or w 0))
        h     (max 1 (or h 0))
        scale (min 1 (/ media-max-w w) (/ media-max-h h))
        sw    (* w scale)
        sh    (* h scale)]
    [(max media-min-w sw) (max media-min-h sh)]))
...
(defn async-media-wrapper [event content {:keys [class default-ratio]} render-fn]
  (r/with-let [room-id      @(re-frame/subscribe [:rooms/active-id])
               ...
               info-w       (:w info)
               info-h       (:h info)
               valid-dims?  (and (number? info-w) (pos? info-w) (number? info-h) (pos? info-h))
               !nat-dims    (r/atom (when valid-dims? [info-w info-h]))
               !blob-url    (r/atom nil)
               on-media-load (fn [e]
                               (when-not @!nat-dims
                                 (let [t  (.-target e)
                                       nw (or (.-naturalWidth t) (.-videoWidth t))
                                       nh (or (.-naturalHeight t) (.-videoHeight t))]
                                   (when (and (pos? nw) (pos? nh))
                                     (reset! !nat-dims [nw nh])))))
               _   (go
    ...
    (let [url   @!blob-url
          dims  @!nat-dims
          style (if dims
                  (let [[tw th] (fit-media-dims (first dims) (second dims))]
                    {:width        (str tw "px")
                     :max-width    "100%"
                     :aspect-ratio (str tw " / " th)
      ...
      [:div.media-container {:class class :style style}
       (if url
         (render-fn url (or (:caption content) (:filename content) "media") on-media-load)
...
(defn calc-media-height [content actual-tag _available-w theme-metrics]
  (let [info          (or (:info content)
        ...
        calc-h        (if valid-dims?
                        (second (fit-media-dims w h))
                        (min media-max-h (/ 300 default-ratio)))
...
(defn sticker-message [event content]
  [async-media-wrapper event content {:class "media-sticker" :default-ratio 1.0}
   (fn [url alt on-load]
     [:img {:src url :alt alt :loading "eager" :on-load on-load
            :style {:width "100%" :height "100%" :display "block"}} ])])
